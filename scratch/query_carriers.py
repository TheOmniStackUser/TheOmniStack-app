...
conn = connect(db_url)
cursor = conn.cursor()
cursor.execute("SELECT environment, client_id, client_secret, api_key, metadata FROM marketplace_integrations WHERE type = 'mirakl_custom' AND metadata->>'customName' ILIKE 'secret sales%' AND is_active = true LIMIT 1")
row = cursor.fetchone()
if not row:
...
if baseUrl.endswith('/v1'):
    baseUrl = baseUrl[:-3]
url = f"{baseUrl}/api/shipping/carriers"
print("Fetching:", url)
...
try:
    ...
    print("Failed to fetch carriers:", e)
