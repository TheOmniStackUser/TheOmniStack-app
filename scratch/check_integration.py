import pg8000.dbapi
import ssl
import json

DATABASE_URL = "postgresql://neondb_owner:npg_iYQt4xBdqH5l@ep-little-band-alr3isna-pooler.c-3.eu-central-1.aws.neon.tech/neondb?sslmode=require"

url = DATABASE_URL.replace("postgresql://", "")
creds, rest = url.split("@")
username, password = creds.split(":")
host_db = rest.split("?")[0]
host, database = host_db.split("/")

ssl_context = ssl.create_default_context()

try:
    conn = pg8000.dbapi.connect(
        user=username,
        password=password,
        host=host,
        database=database,
        ssl_context=ssl_context
    )
    cursor = conn.cursor()
    
    # Get integrations
    cursor.execute(
        "SELECT id, type, auto_invoice, upload_invoice, metadata, is_active FROM marketplace_integrations WHERE company_id = %s",
        ('abe0132f-18e4-41a8-92f7-e65005cfa6aa',)
    )
    integrations = cursor.fetchall()
    print("=== INTEGRATIONS ===")
    for i in integrations:
        print("ID:", i[0])
        print("Type:", i[1])
        print("Auto Invoice:", i[2])
        print("Upload Invoice:", i[3])
        print("Metadata:", json.dumps(i[4], indent=2) if i[4] else None)
        print("Is Active:", i[5])
        print("-" * 30)
        
    cursor.close()
    conn.close()
except Exception as e:
    print("Failed:", e)
