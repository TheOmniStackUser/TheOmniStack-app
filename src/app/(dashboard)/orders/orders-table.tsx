...
const getMarketplaceBadgeStyle = (mp: string) => {
  const lowerMp = (mp || '').toLowerCase()
  if (lowerMp.includes('decathlon')) {
        ...
        <div className="flex items-center gap-4">
          ...
          {selectedIds.size > 0 && (
            <button
              ...
              className="text-xs text-gray-600 hover:text-gray-800 font-semibold flex items-center gap-1.5 border border-gray-200 hover:bg-gray-50 px-2.5 py-1.5 rounded-md bg-white transition-all shadow-sm cursor-pointer"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex flex-wrap items-center gap-3">
          <select
            ...
            onChange={(e) => {
              ...
              pushParams({ marketplace: val === 'all' ? undefined : val, page: '1' })
            }}
          ...
          <select
            ...
            onChange={(e) => {
              const val = e.target.value
              setDraftStatus(val)
              pushParams({ status: val === 'all' ? undefined : val, page: '1' })
            }}
            className="px-3 py-2 border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 min-w-[150px] text-gray-900 font-medium text-sm"
          >
                      ...
                      <div className="flex items-center rounded-lg bg-gray-100 p-0.5 border border-gray-200/60 shadow-inner">
                        <button
                          ...
                          onClick={() => {
                            ...
                            pushParams({ [item.id]: undefined, page: '1' })
                          }}
          ...
          <select
            ...
            onChange={(e) => {
              const val = e.target.value
              setDraftCountry(val)
              pushParams({ country: val === 'all' ? undefined : val, page: '1' })
            }}
          ...
          <select
            ...
            onChange={(e) => {
              ...
              pushParams({ invoiceFilter: val === 'all' ? undefined : val, page: '1' })
            }}
            className="px-3 py-2 border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 min-w-[150px] text-gray-900 font-medium text-sm"
          >
          ...
          <select
            ...
            onChange={(e) => {
              ...
              pushParams({ dateType: val, page: '1' })
            }}
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => {
                ...
                pushParams({ fromDate: val || undefined, page: '1' })
              }}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedOrders.map((order) => {
                ...
                const getShippingStatusDetails = (status: string | null | undefined) => {
                  switch (status) {
                    case 'in_preparation': return { icon: '🛠️', text: 'In Vorbereitung', bg: 'bg-slate-50', border: 'border-slate-200' }
                    case 'in_transit': return { icon: '🚚', text: 'Unterwegs', bg: 'bg-blue-50', border: 'border-blue-200' }
                    case 'delivered': return { icon: '✅', text: 'Zugestellt', bg: 'bg-emerald-50', border: 'border-emerald-200' }
                    case 'ready_for_pickup': return { icon: '📬', text: 'Abholbereit', bg: 'bg-amber-50', border: 'border-amber-200' }
                    case 'delayed': return { icon: '⏳', text: 'Zustellung verzögert', bg: 'bg-purple-50', border: 'border-purple-200' }
                    case 'not_possible': return { icon: '🚫', text: 'Zustellung nicht möglich', bg: 'bg-rose-50', border: 'border-rose-200' }
                    case 'returned': return { icon: '🔄', text: 'Retoure', bg: 'bg-slate-50', border: 'border-slate-200' }
                    default: return { icon: '🛠️', text: 'In Vorbereitung', bg: 'bg-slate-50', border: 'border-slate-200' }
                  }
                };
                          ...
                          {openMenuOrderId === order.id && (
                            <div className="absolute right-0 top-full mt-1.5 w-72 bg-white border border-slate-200 rounded-xl shadow-2xl z-[100] flex flex-col py-1 animate-in fade-in slide-in-from-top-2 duration-150">
                              ...
                              {order.marketplace === 'otto' && (
                                <button
                                  type="button"
                                  onClick={() => setShowOttoRefundModal(order)}
                                  className="flex w-full items-center gap-2 px-4 py-2.5 text-left text-sm text-slate-700 hover:bg-slate-50 transition-colors whitespace-nowrap"
                                >
                          ...
                          <div className="mt-2 space-y-2">
                            <div>
                              ...
                              <input 
                                ...
                                value={editName} 
                                onChange={(e) => setEditName(e.target.value)}
                                className="w-full text-sm !text-slate-900 placeholder:!text-slate-500 p-1.5 border border-slate-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500" 
                              />
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                              <div>
