...
export function KauflandIntegrationForm({ 
  ...
  const [state, action, pending] = useActionState(saveKauflandIntegrationAction, undefined)
    ...
    <form action={action} className="space-y-6 max-w-xl">
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          <label htmlFor="clientId" className="block text-sm font-semibold text-gray-700">Client Key</label>
          <div className="group relative">
            ...
            <div className="absolute left-6 top-0 w-64 p-3 bg-slate-900 text-white text-xs rounded-xl shadow-2xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all z-50 transform -translate-y-1/4">
              ...
              <p className="leading-relaxed text-slate-300">
                Logge dich im <strong>Kaufland Seller Portal</strong> ein. Navigiere zu <strong>Einstellungen &gt; API-Schnittstelle</strong>. Kopiere den <strong>Client Key</strong>.
              </p>
        ...
        <input
          ...
          required
          placeholder="Dein Kaufland Client Key"
      ...
      <div className="space-y-1">
        <label htmlFor="clientSecret" className="block text-sm font-semibold text-gray-700">Secret Key</label>
        <input
          id="clientSecret"
          name="clientSecret"
          type="password"
          required
          placeholder="Dein Kaufland Secret Key"
          className="block w-full px-4 py-2.5 border border-gray-300 rounded-xl shadow-sm text-black focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-white"
        />
