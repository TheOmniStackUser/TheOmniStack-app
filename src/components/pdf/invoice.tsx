...
export const InvoiceDocument: React.FC<InvoiceProps> = ({
  ...
  const t = {
    de: {
      ...
      orderDate: process.env.NEXT_PUBLIC_APP_VARIANT === 'craft' ? 'Auftragsdatum' : 'Bestelldatum',
      buyerRef: 'Käuferref.',
        ...
        <View style={styles.table}>
          ...
          {items.map((item, idx) => (
            <View key={idx} style={styles.tableRow} wrap={false}>
              ...
              <Text style={styles.colPrice}>{formatPrice(item.unitPrice * (1 + item.taxRate) * factor)}</Text>
              <Text style={styles.colTotal}>{formatPrice(item.unitPrice * (1 + item.taxRate) * item.quantity * factor)}</Text>
            </View>
          ))}
        </View>
