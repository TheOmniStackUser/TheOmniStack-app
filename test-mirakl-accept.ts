...
async function main() {
  ...
  console.log(`Trying to accept ${returnId} with global endpoint...`);
  const acceptUrl = `${integration?.environment}/api/returns/accept`;
  const payload = {
    "returns": [
      {
        "id": returnId
  ...
  const res = await fetch(acceptUrl, { method: 'PUT', headers, body: JSON.stringify(payload) });
