  ...
  const handleInlineReducedPriceUpdate = async (productId: string, newValue: string) => {
    try {
      ...
      if (!isNaN(parsed) || newValue === '') {
        ...
        await bulkUpdateStockAndPrice([productId], undefined, undefined, newValue === '' ? 0 : parsed)
                          ...
                          <input
                            ...
                            onKeyDown={async (e) => {
                              if (e.key === 'Enter') {
                                ...
                                if (val !== undefined) {
                                  await handleInlineReducedPriceUpdate(product.id, val)
                                }
                              }
                            }}
                            onBlur={async () => {
...
if (content.includes('className="w-24 px-2 py-1 text-sm font-semibold text-slate-900 border border-slate-200 rounded focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 disabled:opacity-50"')) {
  // Locate tdPrice and append tdReducedPriceNew
...
content = content.replace(priceInputBlock.trim(), priceInputBlock.trim() + '\\n' + reducedPriceInputBlock)
